...
const els = {
  ...
  moodChart: $("#moodChart"),
  moodGrid: $("#moodGrid"),
  scoreScaleContainer: $("#scoreScaleContainer"),
};
...
function renderImagePreview() {
  ...
  els.imagePreviewGrid.innerHTML = pendingImages.map((img, idx) => `
    <div style="position:relative; display:inline-block;">
      <img src="${img}" alt="日记图片" />
      <button type="button" style="position:absolute; top:-6px; right:-6px; background:#e84b3c; color:white; border:none; border-radius:50%; width:20px; height:20px; font-size:12px; cursor:pointer; line-height:20px; text-align:center;" data-imgidx="${idx}">×</button>
    </div>
  `).join("");
  els.imagePreviewGrid.querySelectorAll("[data-imgidx]").forEach(btn => {
...
function renderTodayPulse() {
  ...
  els.todayPulse.innerHTML = `
    <span class="pulse-dot" style="background:${mood.color}; box-shadow:0 0 0 6px ${hexToRgba(mood.color, 0.16)}"></span>
...
function renderVerdict(current, past, baseDate, targetDate) {
  ...
  let verdict = "";
  if (delta >= 3) verdict = "今天明显更开心 🌟";
  else if (delta >= 1) verdict = "今天稍微更轻 ☀️";
  else if (delta === 0) verdict = "今天和那时差不多 ⚖️";
  else if (delta <= -3) verdict = "今天更沉一些 🌧️";
  else if (delta <= -1) verdict = "今天略微低一点 🌫️";
  else verdict = "今天和那时差不多 ⚖️";
  ...
  els.moodVerdict.innerHTML = `
    ...
    <div class="verdict-copy">
      <h3>${verdict}</h3>
      <p>${formatShortDate(current.entry.date)} 与 ${formatShortDate(past.entry.date)} 并排看：</p>
      <p style="font-weight:600; margin-top:4px;">情绪分差 <span style="color:${color};">${delta > 0 ? "+" : ""}${delta}</span> 
      （${pastMood.label} ${pastScore}分 → ${currentMood.label} ${currentScore}分）</p>
      <p style="font-size:0.85rem; color:var(--muted);">${distanceCopy(past, targetDate)}</p>
    </div>
  ...
  target.innerHTML = `
    ...
    <div class="card-body">
      <div class="card-meta">
        ...
        <span class="shift-badge" style="background:${mood.color}">情绪分 ${score}</span>
      </div>
      <h3>${escapeHtml(entry.title)}</h3>
      <div class="entry-content">${escapeHtml(entry.body || "这一天只留下了心情，没有写正文。")}</div>
      ${entry.images && entry.images.length > 0 ? `<div class="entry-images">${entry.images.map(img => `<img src="${img}" alt="日记图片" />`).join("")}</div>` : ""}
...
function renderArchive() {
  ...
  els.entryList.innerHTML = filtered
    .map((entry) => {
      const mood = moodMap[entry.mood];
      const imgPreview = entry.images && entry.images.length > 0 
        ? `<span style="font-size:12px; color:var(--muted);"> 🖼️${entry.images.length}</span>` 
        : "";
      return `
        <article class="entry-row">
          <div class="entry-color" style="background:${mood.color}"></div>
          <div class="entry-summary">
            <h3>${mood.icon} ${escapeHtml(entry.title)} ${imgPreview}</h3>
            <p>${formatLongDate(entry.date)} · ${mood.label} · 情绪分 ${moodScore(entry)} · ${escapeHtml(entry.body.slice(0, 60))}</p>
          </div>
...
function distanceCopy(match, requestedDate) {
  ...
  return `${formatShortDate(requestedDate)} 没有记录，已自动顺延到${Math.abs(match.distance)}天${direction}的 ${formatShortDate(match.entry.date)}。`;
}
...
function renderScoreScale() {
  const sorted = Object.entries(moodMap).sort((a, b) => b[1].score - a[1].score);
  els.scoreScaleContainer.innerHTML = sorted.map(([key, val]) => `
    <span class="scale-item">
      <span class="scale-dot" style="background:${val.color};"></span>
      ${val.icon} ${val.label} (${val.score})
    </span>
  `).join("");
}
...
function renderMoodChart() {
  ...
  if (recent.length < 2) {
    ...
    ctx.clearRect(0, 0, els.moodChart.width, els.moodChart.height);
    return;
  }

  const labels = recent.map(e => formatShortDate(e.date));
  const data = recent.map(e => moodScore(e));
  const colors = recent.map(e => moodMap[e.mood].color);
  ...
  moodChartInstance = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [{
        ...
        data: data,
        borderColor: "#316bff",
        backgroundColor: "rgba(49,107,255,0.1)",
    ...
    options: {
        ...
        tooltip: {
          callbacks: {
            label: function(context) {
              ...
              const mood = moodMap[entry.mood];
              return `${mood.icon} ${mood.label} · 情绪分 ${context.parsed.y} · 强度 ${entry.energy}`;
            }
          }
        }
      },
      scales: {
        y: {
          ...
          max: Math.max(12, Math.max(...data) + 2),
...
function renderMoodGrid() {
  ...
  els.moodGrid.innerHTML = days.map(dateKey => {
    const entry = entries.find(e => e.date === dateKey);
    const mood = entry ? moodMap[entry.mood] : null;
    const color = mood ? mood.color : "#eef0f2";
    const label = mood ? `${mood.icon} ${mood.label}` : "无记录";
    const score = entry ? moodScore(entry) : "";
    
    return `
      <div class="grid-cell ${entry ? 'has-entry' : ''}" 
           style="background:${color};"
           title="${formatLongDate(dateKey)}: ${label}${score ? ' · 情绪分 '+score : ''}">
        ${entry ? `<span class="tooltip-badge">${formatShortDate(dateKey)} ${mood.icon} ${score}</span>` : ''}
      </div>
    `;
  }).join("");
}